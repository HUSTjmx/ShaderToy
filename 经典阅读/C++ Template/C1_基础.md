...
==显式实例化指示符==由关键字`template`和紧接其后的**实例化实体（可以是类、函数、成员函数等）的声明**组成，而且，该声明是一个已经用实参完全替换参数之后的声明。在例子中，我们针对的是一个普通函数，但该指示符也适用于**成员函数和静态数据成员**。譬如：
